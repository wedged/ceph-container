...
function osd_activate {
  ...
  CEPH_ACTIVATE_BINARY=(ceph-volume)
  CEPH_DISK_OPTIONS=()
  ...
  if [[ ${OSD_DMCRYPT} -eq 1 ]] && [[ ${OSD_FILESTORE} -eq 1 ]]; then
    ...
    CEPH_ACTIVATE_BINARY_CLI_OPTS+=('--dmcrypt')
  ...
  if [[ -z "${CEPH_DISK_OPTIONS[*]}" ]]; then
    "${CEPH_ACTIVATE_BINARY[@]}" --setuser ceph --setgroup disk activate --no-start-daemon "${DATA_PART}"
  else
    "${CEPH_ACTIVATE_BINARY[@]}" --setuser ceph --setgroup disk activate "${CEPH_ACTIVATE_BINARY_CLI_OPTS[@]}" --no-start-daemon "${DATA_PART}"
  fi
